...
jQuery.fn.pagination = function(maxentries, opts){
	...
	return this.each(function() {
		...
		function drawLinks() {
            ...
            var toPage='<span><a id="search" href="javascript:;">跳转</a></span>';
            $(pagetext).appendTo(panel);
            $(toPage).appendTo(panel);
            $("#search").bind("click",function(evt){
                var iPageNum = $.trim($("#pagevalue").val()) -1;
                if(iPageNum<np&&iPageNum>-1){pageSelected(iPageNum,evt);}else if(iPageNum>=np){$("#pagevalue").val("");}else{$("#pagevalue").val("")}
